...
import issueAlertTriggeredPayload from './fixtures/issues/alerts/triggered.json';
...
describe('Issue Alerts Webhooks', () => {
    ...
    describe('POST /sentry/webhook', () => {
        it('should handle issue alert triggered webhook', async () => {
            // Send the request
            const response = await request(app)
                .post('/sentry/webhook')
                .set('sentry-hook-resource', 'event_alert')
                .send(issueAlertTriggeredPayload);
